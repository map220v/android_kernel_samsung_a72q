...
export ARCH=arm64
export PROJECT_NAME=a72q
export SEC_BUILD_CONF_SIGNER_MODEL_NAME=SM-A725F_CIS_SER_QKEY0
mkdir out
...
make -j$(nproc) -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CLANG_TRIPLE=$CLANG_TRIPLE vendor/a72q_eur_open_defconfig
